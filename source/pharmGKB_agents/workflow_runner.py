...
class WorkflowRunner:
    ...
    def _convert_app_data_to_inputs(self, output_dict: Dict[str, Any]) -> List[UserInput]:
        ...
        for gene, gene_entries in pharmgkb_evidence.items():
            ...
            print(f"DEBUG: gene_entries type: {type(gene_entries)}")
            print(f"DEBUG: gene_entries content: {gene_entries}")
            
            if not isinstance(gene_entries, list):
                print(f"DEBUG: Skipping {gene} - not a list")
                continue
            ...
            if gene_entries:
                for i, entry in enumerate(gene_entries):
                    if isinstance(entry, dict):
                        ...
                        print(f"DEBUG: Added pharmGKB entry {i+1}")
                    else:
    ...
    def _format_pharmgkb_evidence_item(self, evidence_item: dict, index: int) -> str:
        """Format pharmGKB evidence dictionary into readable string."""
        drugs = evidence_item.get('Drug(s)', 'Unknown drugs')
        sentence = evidence_item.get('Sentence', 'No sentence available')
        notes = evidence_item.get('Notes', '')
        pmid = evidence_item.get('PMID', '')
        
        evidence_str = f"Drugs: {drugs}\nStatement: {sentence}"
